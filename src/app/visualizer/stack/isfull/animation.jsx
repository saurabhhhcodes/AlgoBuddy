...
import { pushGenerator, popGenerator, checkFullGenerator } from "@/features/algorithms/stack/stackIsFullLogic";
...
const StackVisualizer = () => {
  ...
  const [pendingOp, setPendingOp] = useState(null); // { type: 'push', value: string } | { type: 'pop' } | { type: 'isfull' }
  ...
  const frames = useMemo(() => {
    ...
    } else if (pendingOp.type === "isfull") {
      return Array.from(checkFullGenerator(stack, capacity));
    }
    return [];
  ...
  const checkFull = () => {
    if (engine.isPlaying || pendingOp || capacity === null) return;
    setPendingOp({ type: "isfull" });
    setIsFullStatus(null);
    engine.reset();
    engine.play();
  };
    ...
    if (engine.currentStep === frames.length - 1 && frames.length > 0) {
      ...
      if (finalFrame && finalFrame.type === "complete") {
        if (pendingOp?.type === "push" || pendingOp?.type === "pop") {
          ...
          setIsFullStatus(finalFrame.isFull);
        }
      }
      setPendingOp(null);
      engine.reset();
  ...
  return (
    <VisualizerInteractiveLayout>
      <p className="text-center text-lg text-[#6b7280] dark:text-[#9ca3af]">
        Visualize Push, Pop, Peek, and IsFull operations
      </p>
        ...
        {capacity === null ? (
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex gap-4">
              ...
              >
                {engine.isPlaying && pendingOp?.type === "isfull" ? "Checking..." : "IsFull"}
              </button>
              <button
                onClick={pop}
      ...
      <div className="w-full mb-6 max-w-4xl mx-auto p-4 rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm text-center min-h-[60px] flex flex-col items-center justify-center">
        ...
        {isFullStatus !== null && !engine.isPlaying && (
          <div className={`mt-2 font-bold text-lg ${isFullStatus ? 'text-rose-500' : 'text-emerald-500'}`}>
            Result: {isFullStatus ? 'True (Full)' : 'False (Not Full)'}
          </div>
        )}
      </div>
        ...
        {capacity === null ? (
          ...
          <div className="flex flex-col items-center min-h-[300px]">
            ...
            <div className="w-full max-w-md space-y-1.5 relative">
              ...
              <div className={`transition-all duration-500 rounded-xl ${pendingOp?.type === 'isfull' && currentFrame.type === 'start' ? 'ring-4 ring-rose-500 shadow-2xl scale-105' : ''}`}>
              {slots.map((slot) => {
