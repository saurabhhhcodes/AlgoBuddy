...
import { pushGenerator, popGenerator } from "@/features/algorithms/stack/stackLogic";
...
const StackVisualizer = () => {
  ...
  const [pendingOp, setPendingOp] = useState(null); // { type: 'push', value: string } | { type: 'pop' }
  ...
  const slots = [];
  if (capacity !== null) {
    // We want index 0 to be the bottom visually, or index capacity-1 to be the top.
    // In our logic, index 0 is TOP.
    // displayStack[0] is Top element.
    // displayStack[displayStack.length - 1] is Bottom element.
    for (let i = capacity - 1; i >= 0; i--) {
      // i represents the index from bottom (0) to top (capacity - 1)
      const isFilled = i < displayStack.length;
      // If displayStack length is 3 (indices 0,1,2). Bottom is index 2, top is index 0.
      // For visual index i=0 (bottom), we want displayStack[length - 1].
